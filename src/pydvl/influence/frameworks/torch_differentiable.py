...
class TorchTwiceDifferentiable(BaseTwiceDifferentiable[torch.Tensor, nn.Module]):
    ...
    def __init__(
        ...
        """
        :param model: A torch.nn.Module representing a (differentiable) function.
    ...
    def grad(
        ...
        :param y: A matrix [NxK] representing the predicted target values $y_i$.
        :param progress: True, iff progress shall be printed.
    ...
    def mvp(
        ...
        This second order derivative can be on the model parameters or on
        ...
        :param grad_xy: an array [P] holding the gradients of the model
            ...
            self.grad.
        :param v: A np.ndarray [DxP] or a one dimensional np.array [D] which
            multiplies the Hessian, where D is the number of directions.
        :param progress: True, iff progress shall be printed.
        :param backprop_on: tensor used in the second backpropagation (the first
            one is along $x$ and $y$ as defined via grad_xy). If None, the model
            parameters are used.
