...
def calc_s_test(
    ...
    Arguments:
        ...
        damp: float, influence_functions function damping factor
        scale: float, influence_functions calculation scaling factor
...
def load_s_test(
    ...
    """Loads all s_test data required to calculate the influence_functions function
...
def load_grad_z(grad_z_dir=Path("./grad_z/"), train_dataset_size=-1):
    """Loads all grad_z data required to calculate the influence_functions function and
    returns it.
...
def calc_influence_function(train_dataset_size, grad_z_vecs=None, e_s_test=None):
    """Calculates the influence_functions function
    ...
    for i in range(train_dataset_size):
        ...
        display_progress("Calc. influence_functions function: ", i, train_dataset_size)
...
def calc_influence_single(
    ...
    Returns:
        ...
        test_id_num: int, the number of the test dataset point
            the influence_functions was calculated for"""
    ...
    # Calculate the influence_functions function
...
def calc_img_wise(config, model, train_loader, test_loader):
    """Calculates the influence_functions function one test point at a time. Calcualtes
    ...
    # If calculating the influence_functions for a subset of the whole dataset,
    ...
    influences = {}
    # Main loop for calculating the influence_functions function one test sample per
    # iteration.
    for j in range(test_dataset_iter_len):
        ...
        ###########
        # Different from `influence_functions` above
        ###########
        influences[str(i)] = {}
        ...
        influences[str(i)]["influence_functions"] = infl
...
def calc_all_grad_then_test(config, model, train_loader, test_loader):
    """Calculates the influence_functions function by first calculating
    all grad_z, all s_test and then loading them to calc the influence_functions"""
