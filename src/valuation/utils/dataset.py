...
class Dataset:
    ...
    def __init__(self,
        ...
        if x_train.shape[-1] != x_test.shape[-1]:
            raise ValueError(f"Mismatching number of features: "
                             f"{x_train.shape[-1]} and {x_test.shape[-1]}")
        if x_train.shape[0] != y_train.shape[0]:
            raise ValueError(f"Mismatching number of samples: "
                             f"{x_train.shape[-1]} and {x_test.shape[-1]}")
        ...
        self.feature_names = list(feature_names) \
            if feature_names is not None else make_names('x', x_train)
        self.target_names = list(target_names) \
            if target_names is not None else make_names('y', y_train)
        ...
        if len(self.y_train.shape) > 1:
            if len(self.target_names) != self.y_train.shape[-1] \
    ...
    @classmethod
    def from_sklearn(cls, data: Bunch, train_size: float = 0.8,
