...
def combinatorial_shapley(model: SupervisedModel, data: Dataset) -> OrderedDict:
    """ Computes the exact Shapley value. """

    # Arbitrary choice ~= 1.14 hours if 1 sec per fit() + score()
    if len(data) > 12:
        raise ValueError(f"Large dataset! Computation requires {2**len(data)} "
    ...
    @lru_cache
    def utility(indices):
        x = data.x_train[data.index.isin(indices)]
        y = data.y_train[data.index.isin(indices)]
        model.fit(x, y)
