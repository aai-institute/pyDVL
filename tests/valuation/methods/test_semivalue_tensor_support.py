...
from pydvl.valuation.methods.shapley import (
    ShapleyValuation,
...
@pytest.mark.parametrize(
    ...
    [(TensorMinerGame, {"n_players": 4}), (TensorShoesGame, {"left": 2, "right": 1})],
)
...
def test_beta_shapley_valuation_tensor_support(
    game_cls, game_kwargs, alpha, beta, seed
):
...
def test_loo_valuation_tensor_support(game_cls, game_kwargs):
    ...
    # Create the valuation
    valuation = LOOValuation(utility=game.u, progress=False)
...
def test_compare_numpy_tensor_results_deterministic():
    ...
    tensor_data = Dataset(
        x_tensor, nil_tensor.clone(), feature_names=["x"], target_names=["y"]
    )
    ...
    numpy_data = Dataset(
        x_numpy, nil_numpy.copy(), feature_names=["x"], target_names=["y"]
    )
    ...
    tensor_valuation = ShapleyValuation(
        utility=tensor_game.u, sampler=sampler, is_done=stopping, progress=False
    )

    numpy_valuation = ShapleyValuation(
        utility=numpy_game.u, sampler=sampler, is_done=stopping, progress=False
    )
