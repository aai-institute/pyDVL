...
def test_sample_creation_with_tensor():
    ...
    assert torch.is_tensor(sample.subset)
    assert torch.equal(sample.subset, subset)

    # Test with int32 tensor
    subset = torch.tensor([0, 1, 2], dtype=torch.int32)
    sample = Sample(0, subset)
    assert torch.is_tensor(sample.subset)
    assert torch.equal(sample.subset, subset)


def test_sample_with_idx_in_subset_tensor():
    ...
    assert torch.equal(new_sample.subset, torch.tensor([1, 2, 3, 0], dtype=torch.int64))
    
    # Test with idx already in subset
    sample = Sample(1, subset)
    new_sample = sample.with_idx_in_subset()
    assert torch.is_tensor(new_sample.subset)
    assert torch.equal(new_sample.subset, subset)
    assert new_sample is sample  # Should be the same object (no copy)
...
def test_sample_with_subset_tensor():
    ...
    assert torch.is_tensor(new_sample.subset)
    assert torch.equal(new_sample.subset, new_subset)
    
    # Test with same subset (should return same object)
    new_sample = sample.with_subset(subset)
    assert new_sample is sample
...
def test_index_sampler_skip_indices(numpy_indices, torch_indices):
    ...
    class TestSampler(IndexSampler):
        ...
        def generate(self, indices):
            for idx in indices:
                if isinstance(self._skip_indices, np.ndarray) and len(self._skip_indices) > 0:
                    if self._skip_indices[idx]:
                        continue
                elif torch.is_tensor(self._skip_indices) and len(self._skip_indices) > 0:
...
def test_generate_batches_tensor_indices(torch_indices):
    ...
    class TestSampler(IndexSampler):
        ...
        def generate(self, indices):
            for idx in indices:
                yield Sample(idx.item() if torch.is_tensor(idx) else idx, 
                             torch.tensor([], dtype=torch.int64) if torch.is_tensor(indices) 
