...
# noinspection PyTestParametrized
@pytest.mark.parametrize("coefficients", [(np.random.randint(-3, 3, size=5))])
def test_removal(polynomial_dataset, memcache_client_config):
    ...
    x_cont = d.x_train.reshape(
        -1,
    ) + np.random.uniform(-0.05, 0.05, size=len(d))
    x_cont = x_cont[::2]
    ...
    test_indices = np.argsort(d.x_test, axis=0).reshape(
        -1,
    )
    ...
    for i, ypred in enumerate(predicted):
        ...
        plt.ylim(
            min(d.y_train[:n].min(), y_cont.min()) - 1,
    ...
    plt.scatter(d.x_test, d.y_test, label="Test")
    plt.scatter(
        d.x_train[high_to_low][:take],
        d.y_train[high_to_low][:take],
        marker="x",
        label="High value",
    )
