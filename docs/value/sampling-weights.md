...
so-called *sampling strategies*, which subclass 
...
over permutations, or perform whichever computation is required depending on 
the sampling scheme. Crucially, they compute **the product of a semi-value 
coefficient with a sampling weight**. This can be either a form of importance 
sampling, or a mechanism to allow mix-and-matching of sampling strategies and 
semi-value coefficients.
...
$$ w_{\operatorname{unif}} (k) w_{\operatorname{sh}} (k) = \frac{2^{n -
   1}}{n}  \binom{n - 1}{k}^{- 1} $$
...
$$ \int_{0}^1 p (S|q) f (q) \mathrm{d} q = \int_{0}^1 q^k  (1 - q)^{m - k} f
   (q) \mathrm{d} q = p (S) . $$
...
set of indices preceding $i$ in the permutation $\sigma$. We want to determine 
what the correction for permutation sampling $w_{\operatorname{per}}$ and 
Shapley values should be:[^1]
...
it has size $k$. If we denote it as $p (S|k)$ with some abuse of notation, 
then:
...
1. If you mix a generic `ShapleyValuation` with any `PermutationSampler`, the 
   ...
   with $w_{\operatorname{per}} (k) = w_{\operatorname{sh}} (k)^{- 1}$.
1. to obtain the unbiased estimator. This also means that we will always cancel 
   the weights coming from the permutation sampling and can multiply with other 
   coefficients to obtain different methods, for instance 
   `BanzhafValuation`.[^2]
1. Use `TMCShapley`, a class that only uses permutation sampling and sets its 
   valuation coefficient to `None`, thus signalling the Monte Carlo update 
   procedure *not* to apply any correction.
...
correction factors by subclassing from `SemiValueValuation` and setting 
...
[^1]: At step $(\star)$ we have counted the number of permutations before a fixed position of index $i$ and after it, because the utility does not depend on ordering. This allows us to sum over sets instead of permutations.
